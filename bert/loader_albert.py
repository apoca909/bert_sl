...
def albert_params(model_dir, model_name, model_config):
    ...
    """
    config_file = os.path.join(model_dir,  model_config)

    if tf.io.gfile.exists(config_file):
        stock_config = loader.StockBertConfig.from_json_file(config_file)
    elif model_name in albert_models_config:
        albert_config = albert_models_config[model_name]
        stock_config = loader.StockBertConfig(**albert_config)
    else:
        raise ValueError("ALBERT model with name:[{}] not one of tfhub/google-research albert res_models, try one of:{}".format(
            model_name, albert_models_tfhub))
...
def fetch_tfhub_albert_model(albert_model: str, fetch_dir: str, version="2"):
    ...
    local_file_name = "{}.zip".format(name)
...
def map_to_tfhub_albert_variable_name(name, prefix="bert"):
    ...
    name = name.replace("embeddings/word_embeddings_projector/projector",  "encoder/embedding_hidden_mapping_in/kernel")
    name = name.replace("embeddings/word_embeddings_projector/bias", "encoder/embedding_hidden_mapping_in/bias")
...
def load_albert_weights(bert: BertModelLayer, tfhub_model_path, tags=[]):
    ...
    variables = tf.train.list_variables(tfhub_model_path)
    stock_values = OrderedDict()
    for name, shape in variables:
        stock_values[name] = tf.train.load_variable(tfhub_model_path, name)
